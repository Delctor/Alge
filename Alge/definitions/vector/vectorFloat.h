...
namespace alge
{
	...
	public:
		...
		friend inline matrix<float> randomGenerator(matrix<float, false, thisContiguous>&, size_t);

		template<bool matrix1Contiguous, bool matrix2Contiguous>
		friend inline vector<float> kernelDensity(matrix<float, false, matrix1Contiguous>&, matrix<float, false, matrix2Contiguous>&, float);
